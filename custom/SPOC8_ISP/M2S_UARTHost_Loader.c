...
void print_char_n(const char buf[8], int len)
{
    for (int i = 0; i < len; i++)
    {
    	printf ("%c", buf[i]);
...
int main(int argc, char *argv[])
{
	...
	dcb.BaudRate =
			460800;//CBR_57600; // set the baud rate
	...
	{  
        ...
        else if( Action_code[0] == '0')
        {
            if(ret[0] == 'p')
            {
                 printf("\nISP Authentication completed successfully 2\n");
                 return (0);
            }
            else if(ret[0] == 'q')
            {
        ...
        else if( Action_code[0] == '3')
		{
			if(ret[0] == 'p')
			{
				 printf("\nNVM copy direct completed successfully 2\n");
				 return (0);
			}
			else if(ret[0] == 'q')
			{
				printf("\nNVM copy direct failed");
				temp = 0;
				bReadRC =ReadFile(hCom, (char *)&temp, 8,&iBytesRead,NULL);
				printf(" with error code = %d\n",temp);
				return (0);
		...
		printf("Requested return bytes from the target =%d\n", returnbytes);
		...
		while((returnbytes-1)/factor)
		{
		  ...
		  printf ("scrc: 0x%02X ", crc);
		  factor = factor*2;
		}
		bWriteRC = WriteFile(hCom, &crc, 1, &iBytesWritten,NULL);

		char hexstr[24];
		snprintf(hexstr, sizeof(hexstr),
